										...
										<table class='data-table table table-bordered table-striped'
											...
											<tbody>
												<c:forEach var="candidate" items="${candidateinfolist}">
													...
													<td>${candidate.recsubmitdate}</td>
														<td>
															<div class='text-center'>
																<a class='btn btn-default btn-small' data-toggle='modal'
																	href='#modal-status'
																	onclick="doActionstatus('${candidate.candidateid}','${candidate.candidateName}','${jobId}')">
														...
														<td><a class='btn btn-default btn-xs'
															...
															onclick="candidatefeedback_View('${candidate.candidateid}','${candidate.phnNo}','${candidate.candidateName}')">${candidate.phnNo}</a></td>
														<td><div class='text-right'>
																<a href='#modal-candidateDetails' class='btn btn-default btn-xs' data-toggle='modal'
																	onclick="candidateDetails('${candidate.candidateid}','${jobId}')"> <i
														...
														<td>
															<div class='text-center'>
																<a class='btn btn-default btn-small' data-toggle='modal'
																	...
																	onclick="doActionCad('${candidate.candidateid}')">
														...
														<td><c:choose>
																<c:when
																	test="${candidate.file_upload_name == ''}">
																	<div class='text-center'
														...
														<c:choose>
															<c:when
																test='${empty candidate.clientIntStatus}'>
																<td style="color: Black;"><b>N/A</b></td>
															</c:when>
															<c:when
																test='${candidate.clientIntStatus=="Scheduled"}'>
																<td style="color: green;"><b>${candidate.clientIntStatus}</b></td>
															</c:when>
															<c:otherwise>
																<td style="color: red;"><b>${candidate.clientIntStatus}</b></td>
															</c:otherwise>
														</c:choose>
														...
														<c:choose>
															...
															<c:when
																...
																<td style="color: green;"><b>${candidate.clientstatus}</b></td>
															</c:when>
															<c:when
																test='${candidate.clientstatus!="Selected"}'>
																<td style="color: red;"><b>${candidate.clientstatus}</b></td>
															</c:when>
														...
														<c:choose>
															<c:when
																test='${empty candidate.sctstatus}'>
																<td style="color: Black;" id="tl_sctprevent"><b>N/A</b></td>
															</c:when>
													<c:when
																test='${candidate.sctstatus=="Selected"}'>
																<td style="color:green;"><b><a
																data-toggle='modal' href="#viewsctcomntid"
																 onclick="sctViewComment(${candidate.candidateid})" 
																 style="color:red;">${candidate.sctstatus}</a></b></td>
															</c:when>
												<c:when
																test='${candidate.sctstatus!="Selected"}'>
														...
														<c:choose>
															<c:when	test='${empty candidate.bdmstatus}'>
																<td style="color: black;"><b>N/A</b></td>
																</c:when>
															<c:otherwise>
																<td style="color: darkgreen;"><b>${candidate.bdmstatus}</b></td>
														</c:otherwise>
																...
																<td><select class="selectBox"
																	id="tlStatus${candidate.candidateid}"
																	onchange="tlstatusfun(${candidate.candidateid})"
																	required>
													...
													<td id="comments${candidate.candidateid}">
															<div
																id='chooseReason${candidate.candidateid}'
																style="display: none">
																<select class='input-large'  onchange="tlrejectreason(${candidate.candidateid})"
																	id="rejReason${candidate.candidateid}"
															...
															<div id="txtArea${candidate.candidateid}">
																<textarea
															...
															<td>
																<div
																	id="hidecoment${candidate.candidateid}">
																	<a
																	onclick="clickComment('${candidate.candidateid}','${candidate.bdName}','${clientName}')"
																		data-toggle='modal' href="#modal-upload">Comments</a>
																</div>
															</td>
								<%-- 							<td><c:choose>
																	<c:when
																		test="${candidate.bdmstatus=='To Client-SCT Pending'}">
														...
														<td><a class='btn btn-default btn-xs'
															data-toggle='modal'
															onclick="uploadFile('${candidate.candidateid}','${jobId}','${candidate.phnNo}','${candidate.bdName}','${recusername}','${clientName}','${jobCode}','${comapany}')"
															href='#modal-example3'><strong>Upload
																	Document </strong> </a>
														<td><c:choose>

																<c:when
																	test="${candidate.bdmstatus=='To Client'}">
																	<div class='text-center'>
																		<button type='button'
																...
																<c:when test="${candidate.bdmstatus=='To Client-SCT Pending'}">
																...
																<c:choose>	
																	...
																	<div class='text-center'>
																			...
																			id="ststusUpdate${candidate.candidateid}"
																			onclick="saveTLStatus('${candidate.candidateid}',
																			'${candidate.candidateName}','${jobId}','${candidate.tlempno}','${candidate.recEmpNo}','${bdName}')"
																			>SUBMIT</button>
																			</div>
																			</c:when>
																			<c:otherwise>
																...
																<c:otherwise>
																	...
																	<div class='text-center'>
																		...
																		<button class='btn btn-default btn-small'
																			id="ststusUpdate${candidate.candidateid}"
																			onclick="saveTLStatus('${candidate.candidateid}','${candidate.candidateName}','${jobId}','${candidate.tlempno}','${candidate.recEmpNo}','${candidate.bdName}')"
												...
												<form class='form form-horizontal validate-form'
													...
													<div class='span12'>
														<div class='control-group'>
															...
															<input type="hidden" id="sct_bdname" name="sct_bdname">
															<input type="hidden" id="sct_clname" name="sct_clname">
...
<script>
 function candidateDetails(candidateid,jobid)
{

	 $.ajax({
	    	type: "POST",
	        url: "getCandidateInfoByCandidateId",
	        cache: false,
	        async: false, 
	        data: {"candidateid":candidateid,"jobid":jobid} ,            
	        success: function(response){
	        	
	            var candidate=response.candidateInfo;
	           
	        
	 
  $('#cName').text(candidate.first_name);
  $('#cbasicQual').text(candidate.basic_qualification);
  $('#cHQual').text(candidate.highest_qualification);
  $('#cTEXP').text(candidate.total_experience);
  $('#cPSkill').text(candidate.primaryskill);
  $('#cDOB').text(candidate.dob);
  $('#cRExp').text(candidate.relevantExp);
  $('#cDesignation').text(candidate.candDesignsation);
  $('#cCurCtC').text(candidate.currentCTC);
  $('#cExpCtC').text(candidate.expectedCTC);
  $('#cNPeriod').text(candidate.noticeperiod);
  $('#cWLoc').text(candidate.workLocation);
  $('#cCurOrg').text(candidate.current_org+"/"+candidate.doj_cur_org);
  $('#cPAN').text(candidate.pancard);
  $('#cEMail').text(candidate.emailID);
  $('#cnp_negotiable').text(candidate.noticeprd_negotiable);
  $('#desc_np_negotion').text(candidate.desc_np_negotion);
  $('#any_intervw1').text(candidate.intrw_pipeline);
  $('#int_pipeline_com').text(candidate.int_pipeline_com);
  $('#f2f_intrv').text(candidate.f2f_intrv);
  $('#doj_cur_org').text(candidate.doj_cur_org);
  $('#prev_relivd_date').text(candidate.prev_relivd_date);
  $('#prev_start_date').text(candidate.prev_start_date);
  $('#can_native_loc').text(candidate.can_native_loc);
  $('#is_tek_payroll').text(candidate.is_tek_payroll);
  $('#currwork_isPerm').text(candidate.is_working);
  $('#cur_cmpny_name').text(candidate.cur_cmpny_name);
  $('#durationid').text(candidate.duration);
  if(candidate.prev_org=='NO'){
	  $('#prev_org').text(candidate.prev_org);
	  }
  else{
	  $('#prev_org').text(candidate.prev_org+"/"+candidate.prev_start_date+"&&"+candidate.prev_relivd_date);
	  }
  /* if(prev_org=='NO'){
	  $('#prev_org').text(candidate.prev_org);
	  }
  ...
  $('#bank_stmt').text(candidate.bank_stmt);
  if(candidate.bank_name==null){
	  $('#bank_name').text('NA');
	  }else{ $('#bank_name').text(candidate.bank_name);}
 
  if(candidate.bank_loc==null){
	  $('#bank_loc').text('NA');
	  }else{
  $('#bank_loc').text(candidate.bank_loc);
	  }
  $('#cmpny_docs').text(candidate.cmpny_docs);
  $('#job_change').text(candidate.job_change);
  $('#offer_inhand').text(candidate.offer_inhand);
  if(candidate.family_bckgrund=='YES'){
  ...
  if(candidate.spouse_wrk==null){
	  $('#spouse_wrk').text('NA');
	  }else{
		  $('#spouse_wrk').text(candidate.spouse_wrk);
	  }
  if(candidate.kids_info==null){
	  $('#kids_info').text('NA');
	  }else{
		  $('#kids_info').text(candidate.kids_info);
	  }

  $('#resn_reloc').text(candidate.resn_reloc);
  $('#cskypeid').text(candidate.skypeid);

  $('#cnativeloc').text(candidate.can_native_loc);
  $('#minreqjoin').text(candidate.min_timereq_join);
  $('#ccontact_number').text(candidate.contact_number);
  $('#csecskill').text(secskill);
  $('#crep_mgr_name').text(candidate.mgr_official_emailid+"/"+candidate.rep_mgr_name);
  $('#cidProofDetails').text(candidate.idProofDetails);
  $('#cprofessional_Refrnces').text(candidate.professional_Refrnces);
  $('#ccomskillsRatig').text(candidate.comskillsRatig);
  $('#cintrstlevlworkLoc').text(candidate.intrstlevlworkLoc);

 

  $('#modal-candidateDetails').modal('show');
...
<script type="text/javascript">
function clickComment(id,bdname,clname)
{
	 var jobid= $('#jobId').val();
 $("#modal-upload").show();
 $('#candiid').val(id);
 $('#sct_bdname').val(bdname);
 $('#sct_clname').val(clname);
...
<script type="text/javascript">
  function uploadFile(candidateid,jobid,phnno,bdname,recnam,clnt_nam,jobcode,cmpny){
